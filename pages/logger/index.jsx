...
import { getLoggerAPI, editLoggerAPI, createLoggerAPI, deleteLoggerAPI } from '@/API'
import { useRequest } from 'ahooks'
import moment from 'moment/moment'
function Logger() {
    const [fresh, setFresh] = useState(false)
    const {data,loading} = useRequest(getLoggerAPI,{cacheKey:'loggerCache', refreshDeps: [fresh]})
    ...
    async function onDelete(record) {
        const {data:res} = await deleteLoggerAPI({id:record.logger_id})
        if(res.errorCode === 1) {
    ...
    async function editSubmit(formData) {
        if(action === 'edit')
        {
          const {data:res} = await editLoggerAPI({content:formData.edit, id:currentRecord.logger_id})
          if(res.errorCode === 1) {
          ...
          const {data:res} = await createLoggerAPI({content:formData.edit})
          if(res.errorCode === 1) {
